...
class AppStore extends Component {
  state = {activeTabsId: tabsList[0].tabId, searchInput: ''}
  ...
  getSelectedApp = () => {
    const {activeTabsId, searchInput} = this.state
    const filteredProjects = appsList.filter(
      eachProjectDetails =>
        eachProjectDetails.category === activeTabsId &&
  ...
  render() {
    ...
    const filteredProjects = this.getSelectedApp()
            ...
            <ul className="tabs-container">
              {tabsList.map(eachTabItem => (
                <TabItem
                  ...
                  handelChangeTabs={this.handelChangeTabs}
                  isActive={activeTabsId === eachTabItem.tabId}
                />
              ))}
            </ul>
          </div>
